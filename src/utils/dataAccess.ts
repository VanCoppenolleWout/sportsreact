...
export const getClubIdFromName = async (team: string) => {
  const response = await fetch(`${baseURL}/teams?name=${team}`, options).catch(
    handleError,
  )
...
export const getNextFixture = async (
  ...
  const response = await fetch(
    `${baseURL}/fixtures?season=${season}&&team=${team}&next=50`,
    options,
  ).catch(handleError)
